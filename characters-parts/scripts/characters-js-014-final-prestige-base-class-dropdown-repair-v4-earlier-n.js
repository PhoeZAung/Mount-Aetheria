...
(function(){
  ...
  // v36: do not refill class selects on every click/focus; doing so can close or flicker native dropdowns.
  document.addEventListener('click', event => {
    if(event.target?.closest?.('#maClassBuildCard,#maAddMulticlassBtn,#maMulticlassList') && !event.target?.closest?.('.ffd20-modal')) scheduleRefresh(0);
  }, true);
